(define (domain planificacion-contenidos)
  ...
  (:functions
    ...
    (contenidos_planificados ?dia - dia) ; Función que contará los contenidos planificados por día
  )
  ...
  (:action ver
    ...
    :precondition (and
                    ...
                    (< (contenidos_planificados ?dia) 3)
                  )
    :effect (and
              (visto ?contenido)
              (assign (planificado_para ?contenido) (numero_dia ?dia))
              (increase (contenidos_planificados ?dia) 1) 
            )
  )
)
