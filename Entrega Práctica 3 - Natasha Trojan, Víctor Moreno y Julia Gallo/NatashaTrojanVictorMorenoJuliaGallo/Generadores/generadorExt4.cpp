...
int main() {
	...
	ficheroSalida.open("probExt4Generador.pddl");
	ficheroSalida << "(define (problem planificacion-contenidos-ext4)" << endl; 
    ...
    // Duración contenido
    std::uniform_int_distribution<> distrDuracion(15, 90);
    for (const auto& contenido : nombresContenido) {
        int duracion = distrDuracion(eng) / 15 * 15; // Asegurar múltiplos de 15
        ficheroSalida << "    (= (duracion " << contenido << ") " << duracion << ")" << std::endl;
    }
    ficheroSalida << std::endl;

	// Duración días
    for (int i = 0; i < nombresDias.size(); ++i) {
        ficheroSalida << "    (= (minutos_consumidos " << nombresDias[i] << ") 0)" << std::endl;
    }
    ficheroSalida << endl;
