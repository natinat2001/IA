...
int main() {
	...
	ficheroSalida.open("probExt3Generador.pddl");
	ficheroSalida << "(define (problem planificacion-contenidos-ext3)" << endl; 
	...
	// Número de días
    for (int i = 0; i < nombresDias.size(); ++i) {
        ficheroSalida << "    (= (contenidos_planificados " << nombresDias[i] << ") 0)" << std::endl;
    }
